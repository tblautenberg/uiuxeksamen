...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6 flex">
      {/* Sidebar */}
      <div className="w-1/6 bg-gray-800 p-4 rounded-lg shadow-lg">
        <h2 className="text-xl font-bold text-center text-white mb-6">Widgets</h2>
      ...
      <div className="flex-1 ml-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
          ...
          {showAreaChart && (
            <AreaChartWidget title="🌿 Sales Growth" data={sampleData} dataKeyX="name" dataKeyY="value" />
          )}

          {/* Conditional rendering for Bar Chart */}
          {showBarChart && (
            <BarChartWidget title="📉 Sales Distribution" data={sampleData} dataKeyX="name" dataKeyY="value" />
          )}

          {/* Conditional rendering for Composed Chart */}
          {showComposedChart && (
            <ComposedChartWidget title="📊 Monthly Comparison" data={sampleData} dataKeyX="name" dataKeyY="value" />
          )}

          {/* Conditional rendering for Radar Chart */}
          {showRadarChart && (
            <RadarChartWidget title="📡 Weekly Performance by Subject" data={sampleData} />
          )}
        </div>
      </div>
    </div>
